...
3. Confirm the blue GPIO15 LED gives one 800 ms low-battery pulse when the
   estimate is below 10 percent. At higher simulated battery voltages, confirm
   it gives one through four short pulses according to the documented ranges.
4. With USB removed, apply 3.3 V, 3.6 V, 3.9 V, and 4.2 V at `BAT`. At each
   point, wait for the supply and meter to stabilize and record at least ten
   firmware samples.
5. Verify that `voltage_mv` tracks the multimeter monotonically. Record offset,
   ...
   reviewed.
6. Connect a supported rechargeable cell, apply USB power, and repeat the
...
7. Remove the battery. Confirm the firmware remains operational on USB and
   document the observed voltage; do not interpret that value as reliable
   battery-presence detection.
8. Repeat the full procedure for both PCB revisions.
